...
// NOTE: GridEngine.js has been removed - production uses GameEngine ¡÷ gridGenerator ¡÷ symbolDistribution.js
...
// Initialize cascade services (gridEngine removed - use GameController for spins)
// const gridEngine = new GridEngine(); // REMOVED - Use GameController instead
...
if (shouldSkipRedis) {
  console.log('??  Redis disabled - using fallback authentication');
} else {
  (async () => {
    try {
      initializeRedis();
      ...
      if (connected) {
        ...
        console.warn('??  Redis connection failed - authentication will use fallback');
      }
    } catch (error) {
...
app.post('/api/auth/create-session', async (req, res) => {
  try {
    ...
    console.log(`?Ž® Portal session created for player ${decoded.username}`);
...
app.post('/api/auth-spin', async (req, res) => {
  try {
    ...
    console.log(`?Ž° Auth Spin: Player ${player.username} bet $${betAmount}, won $${spinResult.totalWin}`);
...
/* LEGACY ENDPOINT DISABLED - GridEngine removed`r`n// Legacy spin endpoint for backward compatibility
app.post('/api/spin-legacy', authenticate, async (req, res) => {
  try {
    ...
    console.log(`?Ž° Spin request: bet=$${bet}, quickSpin=${quickSpinMode}, freeSpins=${freeSpinsActive}, multiplier=${accumulatedMultiplier}x`);
    ...
    // DISABLED: GridEngine removed - use GameController instead`r`n    // // Generate complete spin result using GridEngine
    ...
    //     );
    // } catch (persistErr) {
    //     console.error('? ï?  Persist spin failed:', persistErr.message);
    // }
...
io.on('connection', (socket) => {
  ...
  socket.on('spin_request', async (data) => {
    console.log('?Ž° WebSocket spin request:', data);
  ...
  socket.on('desync_detected', async (data) => {
    try {
      ...
      console.log(`? ï? Desync detected: session ${syncSessionId}, type ${desyncType}, step ${stepIndex}`);
  ...
  socket.on('subscribe_rtp_alerts', (data) => {
    try {
      console.log(`? ï? Admin ${data.adminId || 'unknown'} subscribed to RTP alerts`);
  ...
  socket.on('unsubscribe_rtp_alerts', (data) => {
    console.log(`? ï? Admin ${data.adminId || socket.admin_id || 'unknown'} unsubscribed from RTP alerts`);
  ...
  socket.on('subscribe_system_alerts', (data) => {
    try {
      console.log(`?š¨ Admin ${data.adminId || 'unknown'} subscribed to system alerts`);
  ...
  socket.on('unsubscribe_system_alerts', (data) => {
    console.log(`?š¨ Admin ${data.adminId || socket.admin_id || 'unknown'} unsubscribed from system alerts`);
    ...
    try {
      ...
      if (rtpMetrics && rtpMetrics.alerts && rtpMetrics.alerts.length > 0) {
        rtpMetrics.alerts.forEach(alert => {
          ...
          console.log(`? ï? RTP Alert broadcasted: ${alert.message}`);
        });
      }
      ...
      if (systemHealth.health.status !== 'healthy') {
        ...
        console.log(`?š¨ System Alert broadcasted: ${systemAlert.message}`);
      }
...
// Optional: Debug portal (non-production environments)
if (process.env.NODE_ENV !== 'production') {
  ...
  if (fs.existsSync(portalStaticPath)) {
    ...
    console.log('? Debug portal UI enabled at /debug/portal');
  }
}
...
if (!isTestEnv) {
  server.listen(PORT, () => {
    ...
    console.log(`?Ž° Infinity Storm Server running on port ${PORT}`);
    console.log(`?? Client URL: ${CLIENT_URL}`);
    console.log('?“¡ WebSocket server ready');
    console.log(`?Ž® Game available at: http://127.0.0.1:${PORT}`);
    console.log('?? Authentication system active');
    console.log(`??ï¸?Admin panel available at: http://127.0.0.1:${PORT}/admin`);
