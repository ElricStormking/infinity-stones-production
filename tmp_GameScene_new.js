...
window.GameScene = class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        if (window.DEBUG) console.log('?Žµ Old BGM system disabled - using centralized BGM management');
        ...
        const keyMListener = () => {
            console.log('?Žµ DEBUG: Manual main BGM start via SafeSound');
        ...
        const keyXListener = () => {
            if (window.DEBUG) {
                console.log('?Žµ DEBUG: DIRECT BGM start (bypassing SafeSound)');
                console.log('?Žµ this.sound:', this.sound);
                console.log('?Žµ Available cache keys:', this.cache.audio.getKeys());
            }
            ...
            try {
                if (this.sound && this.cache.audio.exists('bgm_infinity_storm')) {
                    ...
                    if (window.DEBUG) console.log('?Žµ Direct BGM created:', directBGM);
                    directBGM.play();
                    if (window.DEBUG) console.log('?Žµ Direct BGM play() called');
                } else {
                    if (window.DEBUG) console.log('?Žµ Direct BGM failed - sound or audio not available');
                }
            } catch (error) {
                console.log('?Žµ Direct BGM error:', error);
            }
        };
        ...
        // Start appropriate BGM based on current game state
            if (window.DEBUG) {
                console.log('?Žµ === GAMESCENE CREATED - CHECKING INITIAL BGM ===');
                console.log('?Žµ Free Spins Active:', this.stateManager.freeSpinsData.active);
                console.log('?Žµ Free Spins Count:', this.stateManager.freeSpinsData.count);
            }
        
        // Start initial BGM after a short delay to ensure audio system is ready
        this.time.delayedCall(500, () => {
            if (window.DEBUG) {
                console.log('?Žµ GameScene: Checking for initial BGM startup');
                console.log('?Žµ BGM Initialized:', window.SafeSound.bgmInitialized);
                console.log('?Žµ Current BGM:', window.SafeSound.currentBGM ? window.SafeSound.currentBGM.key : 'None');
            }
            ...
            if (!window.SafeSound.currentBGM) {
                if (this.stateManager.freeSpinsData.active && this.stateManager.freeSpinsData.count > 0) {
                    if (window.DEBUG) console.log('?Žµ GameScene: Starting Free Spins BGM (Free Spins active)');
                    window.SafeSound.startFreeSpinsBGM(this);
                } else {
                    if (window.DEBUG) console.log('?Žµ GameScene: Starting main BGM (Free Spins not active)');
                    window.SafeSound.startMainBGM(this);
                }
            } else {
                if (window.DEBUG) console.log('?Žµ GameScene: BGM already playing, skipping initial BGM setup');
            }
        });
    ...
    async startSpin() {
        ...
        if (!window.SafeSound.currentBGM) {
            console.log('?Žµ First user interaction - starting main BGM');
            window.SafeSound.startMainBGM(this);
        }
    ...
    async processServerSpin() {
        try {
            ...
            if (spinResult.success && spinResult.data) {
                ...
                if (window.DEBUG) {
                    console.log('?Ž° Server spin successful:', spinResult.data);
                }
        ...
        try {
            ...
            fetch(`${apiBase}/api/demo-spin`, {
                ...
                if (res.ok) {
                    ...
                    console.warn('? ï? Demo spin recording responded with non-OK status');
                }
            }).catch(err => {
                console.warn('? ï? Demo spin recording failed:', err.message);
            });
        } catch (error) {
            // Non-fatal
            if (window.DEBUG) console.warn('? ï? Demo spin recording error:', error.message);
        }
    }
    ...
    handleServerSpinResult(data) {
        if (window.DEBUG) {
            console.log('?Ž° Server spin result received:', data);
        }
    ...
    async processServerSpinResult(serverResult) {
        try {
            ...
            if (window.DEBUG) {
                console.log('?Ž° Processing server spin result:', serverResult);
            }
    ...
    async animateServerCascades(cascades) {
        ...
        for (let i = 0; i < cascades.length; i++) {
            ...
            if (window.DEBUG) {
                console.log(`?Ž° Animating cascade ${i + 1}/${cascades.length}:`, cascadeStep);
            }
    ...
    initializeOrientationHandling() {
        ...
        console.log('?Ž® Initializing orientation change handling for GameScene');
    ...
    initializeMobileGestures() {
        ...
        console.log('?Ž® Initializing mobile gesture detection...');
    ...
    isElementInButton(gesture, button) {
        try {
            ...
            console.warn('?Ž® Error checking button bounds:', error);
            return false;
        }
    }
    ...
    handleHoldAutoSpin(gesture) {
        ...
        console.log('?Ž® Hold gesture detected on spin button - starting auto-spin');
    ...
    handleOrientationChange(detail) {
        console.log(`?Ž® GameScene: Orientation changed from ${detail.oldOrientation} to ${detail.newOrientation}`);
    ...
    handleOrientationPause() {
        ...
        console.log('?Ž® GameScene: Pausing game due to orientation change');
    ...
    handleOrientationResume() {
        ...
        console.log('?Ž® GameScene: Resuming game after orientation change');
        this.orientationPaused = false;
        ...
        if (this.wasAutoplayActive) {
            console.log('?Ž® Auto-play was active before orientation change. Please restart it manually.');
            this.wasAutoplayActive = false;
        }
        
        if (this.wasBurstAutoSpinning) {
            console.log('?Ž® Burst auto-spin was active before orientation change. Please restart it manually.');
